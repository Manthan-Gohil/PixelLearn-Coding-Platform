...
export default function ChapterItem({
    ...
    const chapterCompleted = chapter.exercises.filter((ex) => isExerciseCompleted(ex.id)).length;
    const chapterTotal = chapter.exercises.length;
    const isLocked = chapter.isPremium && userSubscription === "free";
            ...
            >
                ...
                <div className="flex-1 text-left">
                    <div className="flex items-center gap-2">
                        ...
                        {chapter.isPremium && (
                            <span className="px-1.5 py-0.5 rounded text-xs font-medium gradient-primary text-white">PRO</span>
                        )}
                    </div>
            ...
            {isExpanded && (
                <div className="border-t border-border">
                    {chapter.exercises.map((exercise) => {
                        ...
                        return (
                            <div key={exercise.id} className="flex items-center gap-4 px-4 py-3 border-b border-border/50 last:border-0 hover:bg-surface-hover/30 transition-colors">
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${completed ? "bg-success text-white" : canAccess ? "border-2 border-primary/40" : "border-2 border-border"}`}>
                                ...
                                <div className="flex items-center gap-3">
                                    <span className={`text-xs px-2 py-0.5 rounded ${(exercise.difficulty === 'easy' ? "text-success bg-success/10" : exercise.difficulty === 'medium' ? "text-warning bg-warning/10" : "text-danger bg-danger/10")}`}>
                                        {exercise.difficulty}
                                    </span>
                                    ...
                                    {canAccess ? (
                                        <Link
                                            ...
                                            className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${completed ? "border border-success/30 text-success hover:bg-success/10" : "gradient-primary text-white hover:opacity-90"}`}
                                        >
